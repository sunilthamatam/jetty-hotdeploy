...
import org.sthamatam.server.handler.AdminHandler;
import org.sthamatam.server.handler.DeployHandler;
...
//		handlerCollection.addHandler(rootHandler);
		...
		ServerContext serverContext = new LocalServerContext(server);
        server.start();
		// fastest way to stop the server and jvm
		// server.join();
		serverContext.await();
		server.stop();
		// kill as deamon threads block the VM
		// System.exit(0);
    }
}
